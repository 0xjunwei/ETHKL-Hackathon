...
async fn main() {
    ...
    // Define the regular expression pattern for an NRIC
    let re = Regex::new(r"(?i)[STFG]\d{7}[A-Z]").unwrap();
    let mut nric_str = String::new(); // Define the variable outside of the conditional block
    let mut nric_found = false;
    // Search for the NRIC pattern in the response string
    if let Some(captures) = re.captures(&response_str) {
        if let Some(nric) = captures.get(0) {
            nric_str = nric.as_str().to_string();
            nric_found = true;
            println!("NRIC found: {}", nric_str);
        } else {
            println!("No NRIC found");
        }
    } else {
        println!("No NRIC found");
    }
    ...
    if nric_found {
        ...
        let (private_ranges, _) = find_ranges(
            prover.recv_transcript().data(),
            &[
                // redact the nric value
                nric_str.as_bytes(),
            ],
        );
        ...
        let recv_commitment: Vec<_> = private_ranges
            .iter()
        ...
        for commitment_id2 in recv_commitment {
            proof_builder.reveal(commitment_id2).unwrap();
        }
